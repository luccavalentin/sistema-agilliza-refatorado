...
export function DashboardDetailProvider({ children }: { children: ReactNode }) {
  const [ctx, setCtx] = useState<DetailContext | null>(null);
  const [search, setSearch] = useState("");
  const printRef = useRef<HTMLDivElement | null>(null);

  const value = useMemo<Ctx>(() => ({ open: (c) => { setSearch(""); setCtx(c); } }), []);
  ...
  return (
    <DetailCtx.Provider value={value}>
      {children}
      <Dialog open={!!ctx} onOpenChange={(o) => !o && setCtx(null)}>
        <DialogContent className="max-w-5xl gap-0 p-0">
          {ctx && (
            <div className="flex max-h-[88vh] flex-col">
              <header className="flex items-start justify-between gap-3 border-b border-border px-6 py-4">
                <div>
                  <h2 className="text-lg font-bold tracking-tight text-graphite">{ctx.title}</h2>
                  {ctx.subtitle && (
              ...
              <div ref={printRef} className="space-y-4 overflow-y-auto px-6 py-4">
                <h1 style={{ display: "none" }}>{ctx.title}</h1>
                ...
                <section className="kpis grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  {ctx.kpis.map((k) => (
                    <div key={k.label} className="kpi rounded-md border border-border bg-card p-3">
