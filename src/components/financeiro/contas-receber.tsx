/**
 * Contas a Receber — Módulo financeiro do correspondente.
 * TODO: substituir dados mock por query Supabase — tabela `lancamentos_financeiros`
 */
import { useMemo, useState } from "react";
import {
  ArrowDownCircle, CheckCircle2, ChevronLeft, ChevronRight,
...
import { comissoes, contasReceber } from "@/lib/financeiro/mock-data";
...
type StatusFiltro = "todos" | "aberto" | "recebido" | "vencido" | "parcial";

const PAGE_SIZE = 8;

function statusTone(s: string) {
  if (s === "Pago" || s === "Recebido") return "bg-emerald-50 text-emerald-700 border-emerald-200";
  if (s === "Vencido") return "bg-red-50 text-red-700 border-red-200";
  if (s === "Recebido parcialmente" || s === "Pago parcialmente" || s === "Parcial") return "bg-amber-50 text-amber-700 border-amber-200";
  if (s === "Em negociação") return "bg-purple-50 text-purple-700 border-purple-200";
...
export function ContasReceber() {
  ...
  const lancamentos: Lancamento[] = useMemo(() =>
    [...contasReceber].sort((a, b) => new Date(a.vencimento).getTime() - new Date(b.vencimento).getTime()),
    [],
  );
  ...
  const filtrados = useMemo(() => lancamentos.filter((l) => {
    ...
    const matchStatus = statusFiltro === "todos" ||
      ...
      (statusFiltro === "recebido" && (l.status === "Pago" || l.status === "Recebido")) ||
      (statusFiltro === "vencido" && l.status === "Vencido") ||
      (statusFiltro === "parcial" && (l.status === "Recebido parcialmente" || l.status === "Pago parcialmente"));
  ...
  const totalRecebidoMes = lancamentos.filter((l) => (l.status === "Pago" || l.status === "Recebido") && new Date(l.vencimento).getMonth() === new Date().getMonth()).reduce((a, b) => a + b.valor, 0);
  ...
  return (
    <div className="space-y-5">
      <PanelHeader
        ...
        subtitle="Gerencie todas as receitas, comissões e recebíveis do correspondente."
        right={
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ...
          { label: "Recebidos (mês)", value: formatBRL(totalRecebidoMes), accent: "#00b35a", icon: CheckCircle2 },
        ].map((k) => {
          const Icon = k.icon;
          return (
      ...
      <section className="flex flex-wrap items-center gap-3 rounded-lg border border-border bg-card p-4">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            type="search"
            placeholder="Buscar por descrição, parte ou categoria..."
            value={busca}
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border bg-secondary/50">
                ...
                {["Descrição", "Parte / Origem", "Categoria", "Vencimento", "Valor", "Status", "Conta", "Ações"].map((h) => (
                ...
                return (
                  <tr key={l.id} className={`group hover:bg-secondary/30 ${selecionados.has(l.id) ? "bg-brand/5" : ""}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="grid h-7 w-7 shrink-0 place-items-center rounded-md bg-emerald-50 text-emerald-700">
                    ...
                    <td className="px-4 py-3 font-bold text-emerald-700">{formatBRL(l.valor)}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button className="rounded border border-emerald-200 bg-emerald-50 px-2 py-1 text-[10px] font-semibold text-emerald-700 hover:bg-emerald-100">
                          Receber
                        </button>
